...
def match_bonsai_and_imec_pulses(n_pulses_and_samples, data_dir):    
    ...
    spikeglx_pulses = load_imec_pulses(n_pulses_and_samples, directory=spikeglx_dir)
    # n_imec_trials is equal to the number of datasets in imec_pulses
    n_imec_trials = len(spikeglx_pulses)
    ...
    start_sample = 0
    for i, (_, row) in enumerate(n_pulses_and_samples.iterrows()):
        trial_time = row['csv_files']
        bonsai_file = os.path.join(bonsai_dir, 'pulseTS_' + trial_time + '.csv')       
        
        # read the bonsai pulses
        pulses[trial_time] = pd.read_csv(bonsai_file, header=None)
        # name the column 'bonsai_pulses'
        n_pulses[trial_time] = (len(pulses[trial_time]))

        # convert bonsai_pulses to seconds
        bonsai_seconds = pulses[trial_time]/1000
        # convert bonsai_seconds to a list
        bonsai_seconds = bonsai_seconds.values.flatten()
        bonsai_seconds = bonsai_seconds - bonsai_seconds[0]
        print('bonsai seconds: ' + str(bonsai_seconds[0:10]))

        imec_trial_pulses = spikeglx_pulses[i] + start_sample
        ...
        start_sample = start_sample + row['n_samples']
...
def load_imec_pulses(n_pulses_and_samples, directory=None): # these are the pulses generated by extract_pulses_from_raw.py
    ...
    # use tkinter to select the file
        ...
        pulse_file = os.path.join(directory, 'spikeglx_pulses.hdf5')
    ...
    with h5py.File(pulse_file, 'r') as hf:
        ...
        # keys are in the dir_name column of n_pulses_and_samples
        keys = n_pulses_and_samples['dir_name']
...
if __name__ == "__main__":
    ...
    spikeglx_dir = os.path.join(data_dir, 'spikeglx_data')
    # load spike_glx_n_samples_and_pulses.csv into a dataframe
    n_pulses_and_samples = pd.read_csv(os.path.join(spikeglx_dir, 'spikeglx_n_samples_and_pulses.csv'))
    # n_pulses_and_samples = load_pickle('n_pulses_and_samples.pkl', os.path.join(data_dir, 'imec_files'))
    # imec_pulses = load_imec_pulses(n_pulses_and_samples, directory=spikeglx_dir)

    # print the entire dataframe for visual inspection
    print(n_pulses_and_samples)

    # get the list of csv files in the behaviour directory
    behaviour_dir = os.path.join(data_dir, 'behaviour')
    csv_files = glob.glob(os.path.join(behaviour_dir, '*.csv'))
    # make sure these are ordered by time, which directly precedes the '.csv' in the file name, 
    # and is in the format 'hh.mm.ss'
    csv_files = sorted(csv_files, key=lambda x: x[-11:-4])
    # remove the paths from the csv_files
    csv_files = [os.path.basename(file) for file in csv_files]
    # remove the .csv extension
    csv_files = [file[:-4] for file in csv_files]

    # verify that the number of csv files is equal to the number of rows in pulses_df
    assert len(csv_files) == n_pulses_and_samples.shape[0]
    # make a "csv_files" column in pulses_df
    n_pulses_and_samples['csv_files'] = csv_files
    # resave the df as a csv file
    n_pulses_and_samples.to_csv(os.path.join(spikeglx_dir, 'spikeglx_n_samples_and_pulses.csv'), index=False)
