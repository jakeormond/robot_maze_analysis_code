...
def probe_positions_by_hpc_region():
...
def separate_clusters_by_region_and_shank(good_clusters, channel_positions, region_depth):
    ...
    good_clusters['shank'] = -1
    good_clusters['region'] = ''

    for cluster in good_clusters.iterrows():
        ...
        for shank, x_positions in shank_x_positions.items():
            if cluster_position[0] == x_positions[0] or cluster_position[0] == x_positions[1]:
                good_clusters.loc[cluster[0], 'shank'] = int(shank)
                break
        
        for region in regions:
            if pd.notna(region_depth.loc[region, str(shank)]):
                depth = ast.literal_eval(region_depth.loc[region, str(shank)])
                ...
                if cluster_position[1] >= lower_tip_distance and cluster_position[1] <= upper_tip_distance:
                    good_clusters.loc[cluster[0], 'region'] = region
                    break
...
if __name__ == "__main__":
    ...
    # region_depth = probe_positions_by_hpc_region()
    ...
    separate_clusters_by_region_and_shank(good_clusters, channel_positions, region_depth)  
